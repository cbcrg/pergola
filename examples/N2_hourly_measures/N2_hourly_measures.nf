...
process pheno_features_to_pergola {
	container 'cbcrg/pergola:latest'
  	...
  	cat ${name_file}".no_na.bed" | grep -v "track name" > ${name_file}".no_tr.bed.tmp" || echo -e "chr1\t0\t100\t.\t-10000\t+\t0\t100\t135,206,250" > ${name_file}".no_tr.bed.tmp"  	
  	awk '{ print \$0, \"\\t$hour\" }' ${name_file}".no_tr.bed.tmp" > ${name_file}".no_tr.bed" 
  	
  	rm bed_file.tmp
  	rm ${name_file}".no_tr.bed.tmp"
...
/*
 * Grouping files by hour for plots
 */ 
bed_measure_plot_col = bed_measure_no_track_line.collectFile (newLine: false, sort:'none') {
	//def name = "N2." + it[3] + "." + it[1] //it[3] shouldn't be included is the hour 
	def name = "N2." + it[1] 
	[ name, it[2].text ] 
}
...
process plot_distro {
	container 'joseespinosa/docker-r-ggplot2:v0.1'
  	...
  	input:
  	file N2_measure_by_h from bed_measure_plot_col
  	...
  	"""
  	plot_N2_feature_by_hour.R --bed_file=${N2_measure_by_h}
  	"""
}
