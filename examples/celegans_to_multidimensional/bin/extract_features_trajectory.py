...
# for h in list_header [4:]:
#     print "behavioural_file:%s > pergola:dummy" % h

# behavioural_file:frame_start > pergola:start
...
mean_list = list ()
mean_list.append (np.nanmean(np.absolute(np.array(length_worm)))) 
mean_list.append (np.nanmean(np.absolute(np.array(path_range)))) 
mean_list.append (np.nanmean(np.absolute(np.array(eccentricity))))
mean_list.append (np.nanmean(np.absolute(np.array(wave_length_primary))))
mean_list.append (np.nanmean(np.absolute(np.array(kinks))))
mean_list.append (np.nanmean(np.absolute(np.array(track_length))))
print mean_list
# [length_worm, path_range, eccentricity, wave_length_primary, kinks, track_length]

## Checkings and mean calculation just once
d_velocity_mean = {}
 
for body_part in f['worm']['locomotion']['velocity']:
    
    d_velocity_mean[body_part] = {}       
    ...
    for k in f['worm']['locomotion']['velocity'][body_part]:             
        ...
        d_velocity_mean[body_part][k] = np.nanmean(np.absolute(np.array(list_feature)))

d_bends_mean = {}

for body_part in f['worm']['locomotion']['bends']:
    
    d_bends_mean[body_part] = {}
    ...
    for k in f['worm']['locomotion']['bends'][body_part]:                    
        ...
        d_bends_mean[body_part][k] = np.nanmean(np.absolute(np.array(list_feature_b)))

# print d_velocity_mean
# print "++++++++++"
# print d_bends_mean
# exit ("test finished")            
...
# for frame in range(0, 100):    
    ...
    for idx, list_feature in enumerate([length_worm, path_range, eccentricity, wave_length_primary, kinks, track_length]):        
        if np.isnan(np.absolute(np.array(list_feature)[frame])):
            list_v.append (mean_list[idx])
        
        else:
            list_v.append (np.absolute(np.array(list_feature).item(frame)))
#             print np.absolute(np.array(list_feature).item(frame))
...
#                                 % ("velocity_" + body_part + "_" + k))
            ...
            if np.isnan(np.absolute(np.array(list_feature)[frame])):                
                list_v.append (d_velocity_mean[body_part][k])        
            else:
                list_v.append (np.absolute(np.array(list_feature).item(frame)))
...
#                                 % ("velocity_" + body_part + "_" + k))
            ...
            if np.isnan(np.absolute(np.array(list_feature_b)[frame])):
                list_v.append (d_bends_mean[body_part][k])        
            else:
                list_v.append (np.absolute(np.array(list_feature_b).item(frame)))
