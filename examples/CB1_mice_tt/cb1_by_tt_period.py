    ...
    for data_type, dict_bed in dict_exp_gr.iteritems():
        for tr, bed in dict_bed.iteritems(): 
            ...
            for key, bed_phase in d_exp_phases_bed.iteritems():
                ...
                bouts_bed = bed_BedTools.intersect(pybedtools.BedTool(phase + ".bed"))
                
#                 if bouts_bed.count() == 0: 
#                     # When there is any interval we set the mean to zero
#                     list_no_intervals = [("chr1", 0, 1, "no_intervals", 0)]
#                     bed_no_intervals = pybedtools.BedTool(list_no_intervals).saveas('tr_' + exp_group + '.' + '.'.join(tr) + ".NOPHASE." + phase + ".bed")  
#                 else: 
#                     bed_full_length.map(light_bouts_bed, c=5, o=statistic, null=0).saveas ('tr_' + exp_group + '.' + '.'.join(tr) + ".NOPHASE." + phase + ".bed")
# #                  
...
script_path = base_dir + "/bin/plots_CB1_phase_tt.R"
